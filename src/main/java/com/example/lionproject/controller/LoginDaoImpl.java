...
{ 	
	...
	{
	if(com.example.lionproject.dao.DBhelper.SearchMessageExit("user","number",number))return false;//号码已经存在，插入失�?
	//否则，可以插?
	...
	if(!com.example.lionproject.dao.DBhelper.Insert("user", col,info))return false;//插入失败
	return true;//否则，插入成功
}
	public boolean MatchInfo(String number,String password) {
		String[] col= {"number","password"};
		String[] info= {number,password};
		if(com.example.lionproject.dao.DBhelper.MatchInfo("user",col , info))
		return true;
		else return false;
	...
	{
		if(com.example.lionproject.dao.DBhelper.UpdateNameByPhone("user", phone, name))
			return true;
		else return false;
	}
